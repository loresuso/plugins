...
int64_t hash_calculator::checksum(const std::string &filename, hash_type type,
                                  std::string *hash) {
  uint64_t size;
  struct stat s;
  MD5_CTX cmd5;
  ...
  //
  // note: mmap can be risky on 32 bits systems, you can end up exhausting
  // virtual memory easily. Furthermore, always remember to munmap.
  ...
  tmps[2] = 0;
  uint32_t digest_len =
      (type == HT_SHA256) ? SHA256_DIGEST_LENGTH : MD5_DIGEST_LENGTH;
